...
public class Core(SimpleLogger logger, ProgressReporter progressReporter)
{
    ...
    /// <summary>
    /// Decrypts one single blob, and returns encrypted result.
    /// </summary>
    /// <param name="blob">The byte array contains save data to be encrypted.</param>
    /// <param name="gamingPlatform">An implementation of IGamingPlatform that provides platform-specific logic.</param>
    public byte[] DecryptBlob(byte[] blob, IGamingPlatform gamingPlatform)
    {
        var userIdOutput = gamingPlatform.GetParsedUserIdInput();
        // Process file data
        var mandarinFile = new MandarinFile(Deencryptor, MandarinFileFlavor);
        try
        {
        ...
        try
        {
            mandarinFile.DecryptFile(userIdOutput);
        }
        catch (Exception ex)
        {
            return blob;
    ...
    public bool IsEncrypted(byte[] blob, IGamingPlatform gamingPlatform)
    {
        var userIdOutput = gamingPlatform.GetParsedUserIdInput();
        // Process file data
        var mandarinFile = new MandarinFile(Deencryptor, MandarinFileFlavor);
        try
        {
            ...
            return mandarinFile.IsEncrypted;
        }
        catch (Exception ex)
        {
            return true;
    ...
    /// <summary>
    /// Encrypts one single blob, and returns encrypted result.
    /// </summary>
    /// <param name="blob">The byte array contains save data to be encrypted.</param>
    /// <param name="gamingPlatform">An implementation of IGamingPlatform that provides platform-specific logic.</param>
    public byte[] EncryptBlob(byte[] blob, IGamingPlatform gamingPlatform)
    {
        var userIdOutput = gamingPlatform.GetParsedUserIdOutput();
        // Process file data
        var mandarinFile = new MandarinFile(Deencryptor, MandarinFileFlavor);
